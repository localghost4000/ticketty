...
RSpec.describe CommentMailer, type: :mailer do
  describe "new_comment" do
    ...
    let(:email) do
      CommentMailer.new_comment(comment, ticket_owner)
    end
    ...
    it "sends out an email notification about a new comment" do
      ...
      expect(email.text_part.body.to_s).to include title
      expect(email.text_part.body.to_s).to include "#{commenter.email} wrote:"
      expect(email.text_part.body.to_s).to include comment.text
    end
  end
end
